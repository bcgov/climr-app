...
# create queries
query_datasets <- "CREATE TABLE IF NOT EXISTS flp_ts_datasets (
                    region varchar(20),
                    dataset_id SMALLINT,
                    type_id SMALLINT,
                    var_id SMALLINT,
...
query_hist <- "CREATE TABLE IF NOT EXISTS flp_ts_hist (
                    region varchar(20),
                    ssp_id SMALLINT,
                    type_id SMALLINT,
                    var_id SMALLINT,
...
query_proj <- "CREATE TABLE IF NOT EXISTS flp_ts_proj (
                    region varchar(20),
                    ssp_id SMALLINT,
                    type_id SMALLINT,
                    var_id SMALLINT,
...
query_update_ds <- "ALTER TABLE flp_ts_datasets
                    ALTER COLUMN region TYPE varchar(35)"

query_update_hist <- "ALTER TABLE flp_ts_hist
                    ALTER COLUMN region TYPE varchar(35)"

query_update_proj <- "ALTER TABLE flp_ts_proj
                    ALTER COLUMN region TYPE varchar(35)"

dbExecute(dbCon, query_update_ds)
dbExecute(dbCon, query_update_hist)
dbExecute(dbCon, query_update_proj)
...
#unique(samps$ORG_UNIT)
for (flp_code in c(unique(samps$ORG_UNIT))) {
  message("Processing ", flp_code)
  sampsub <- samps[samps$ORG_UNIT == flp_code]
...
dbExecute(dbCon, "grant select on all tables in schema public to readaccess")
poolClose(dbCon)