...
For instructional videos, <a href="https://www.youtube.com/playlist?list=PLt-XQAbbH_-pRO0usT5dHjTt1WidQZIH7" target="_blank" rel="noopener noreferrer">click here</a>.

## Get Data:

In the first tab of the **climr app**, users can select points or areas of interest, choose parameters, downscale the data, and preview and download the data.  For a very quick summary of functionality of this page, users can also click the *What does this page do* link at the top of the left-hand panel. Throughout the app, there are also blue underlined phrases. Click on these hyperlinks to be brought to the relevant help pages.

```{r}
#| label: fig-Get_Data
#| fig-cap: "Get Data tab"
#| fig-align: center
#| out.width: 100%
#| echo: false
knitr::include_graphics("Figure_Instructions_Get_Data.png")
```
...
Under the **Observed Climate Data** accordion, users can select observed periods (the reference period 1961-1990 will always be computed). If the input is individual map points, users can choose to specify observed years and observation time-series data (@fig-Observed_Climate_Data).

```{r}
#| label: fig-Observed_Climate_Data
#| fig-cap: "Example of selected parameters for Observed Climate Data for map point input"
#| fig-align: center
#| out.width: 90%
#| echo: false
...
Click on the **Choose Output Preferences** button. If the area of interest are map point(s) or an uploaded file of single points, the only output format available for the downscaled data is csv. If the area of interest is a map drawn shape or a shape file, users can choose between csv and GeoTIFF as the output format, and choose to include the elevation map in the putput.
...
Click on the green **Generate Results** button. Notification boxes will appear on the right-hand side of the screen while the downscale is processing, indicating each step.

### **Step 5.** Preview downscaled data:

If csv has been selected, a preview will appear under **Step 5** in the main panel beside the map, and users can choose to download the data by clicking on the **Download Downscaled Data** button (@fig-csv_preview).
...
To clear all selections on the map and start again with new data, click on the red **Reset App** button in the main panel.

Please note that users will not be able to open GeoTIFF downloads in Photos. We recommend reading the output into RStudio or opening the file in GIS software.
...
After clicking **Choose Output Preferences**, the available range for downscale resolution is dependent on how many raster layers will be output (@fig-low-res). If a user wants a higher resolution (the highest possible is 250m), we recommend downscaling one GCM at a time.
...
To adjust the plot parameters from the recommended defaults, click on the **Adjust Plot** button, select the observational dataset(s) and SSP(s), and click **OK!** (@fig-Timeseries_ecoregion_parameters).
...
#| out.width: 100%
#| echo: false
knitr::include_graphics("Figure_Instructions_Timeseries_mappoint_parameters.png")
```
...
Click **Load**. The map will take about 5-10 seconds to render (see @fig-Overlay_Tave_2500m for 2500m resolution and @fig-Overlay_Tave_800m for 800m resolution).

```{r}
#| label: fig-Overlay_Tave_2500m
#| fig-cap: "2500m resolution climate map of annual Mean temperature (in °C) across North America"
#| fig-align: center
#| out.width: 100%
#| echo: false
knitr::include_graphics("Figure_Instructions_Overlay_Tave_2500m.png")
```

For ratio variables, an **Apply scale adjustment** checkbox will appear, which applies a log scale adjustment to the data (@fig-Overlay_Tave_800m).

```{r}
#| label: fig-Overlay_Tave_800m
#| fig-cap: "800m resolution climate map of annual Mean precipitation (in mm) across the west coast of North America (with log scale adjustment)"
#| fig-align: center
#| out.width: 100%
#| echo: false
knitr::include_graphics("Figure_Instructions_Overlay_PPT_800m.png")
```

Users can also zoom in on an area they would like to see in more detail, and click on **Rescale**. This will distribute the colour palette across the area of the map that is visible in the screen, allowing the user to see the differences in the variable selected more clearly. The difference between the original overlay and the rescaled version is shown in @fig-Overlay_Tave_800m_unscaled (without rescaling) and @fig-Overlay_Tave_800m_rescaled (with rescaling). Please note that users must **Load** the overlay of interest first, before clicking **Rescale**!

```{r}
#| label: fig-Overlay_Tave_800m_unscaled
#| fig-cap: "800m resolution climate map of annual Mean temperature (in °C) across North America, zoomed into Vancouver Island without rescaling"
#| fig-align: center
#| out.width: 100%
#| echo: false
knitr::include_graphics("Figure_Instructions_Overlay_Tave_800m_unscaled.png")
```

```{r}
#| label: fig-Overlay_Tave_800m_rescaled
#| fig-cap: "800m resolution climate map of annual Mean temperature (in °C) across North America, zoomed into Vancouver Island with rescaling"
#| fig-align: center
#| out.width: 100%
#| echo: false
knitr::include_graphics("Figure_Instructions_Overlay_Tave_800m_rescaled.png")
```
